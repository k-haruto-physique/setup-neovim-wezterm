...
## 28. 自動更新前から開いている Claude セッションが、ブリッジで「待受なし」になる（2026-09-15）

### 症状

B14 の結合テストで、実在する Claude セッションを指す起動記録を置いたのに、`Codex/ask-claude.ps1` が「待受中のセッションなし」と判定した。

### 原因

Claude Code の自動更新は、**動いている実行ファイルを `claude.exe.old.<数字>` に改名**してから新しい `claude.exe` を置く。更新前から開いているセッションのプロセス名は `claude.exe.old.1789433486565` のようになる（実測: 20260915_IG のセッション）。ブリッジは名前が `claude` のプロセスだけを Claude と見なしていたので、次の 2 つが起きていた。

- `ask-claude.ps1`: 待受中でも、送り先の候補から外す。
- `claude-listen.ps1`: 親プロセスをたどっても `claude.exe` が見つからず、待受を起動できない。

### 対処

名前の判定を `claude.exe` と `claude.exe.old.<数字>` の両方に一致させた。PID の使い回しで別のプロセスへ届く心配は、開始時刻の照合で防いでいるので、名前の判定を緩めても安全。
