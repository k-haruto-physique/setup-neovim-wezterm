...
最終更新: 2026-09-15（Claude ⇄ Codex の自走ルールと上限を追加し、3 往復の自走テストに成功。テストで Codex が指摘した弱点のうち、使い勝手とのトレードオフがある 2 件を B14・B15 としてユーザー判断に回した。B13 は CLOSED。**OPEN は2件**）。
...
| # | タスク | 状態 | 次の一手 | 起点 |
|---|---|---|---|---|
| B14 | **ブリッジの送り先があいまいな時に送らないようにするか** | 今は、同じ cwd に複数のセッションがあると最新を自動で選び、警告だけ出して送る（誤配送のリスク）。送らない方式にすると、同じリポジトリで Codex を 2 つ並べる運用（#19）では、毎回 `-ThreadId` の指定が必要になる | ユーザーが「誤配送防止を優先して送らない」か「今のまま（警告付きで自動選択）」かを決める → `ask-codex.ps1`・`ask-claude.ps1` の候補選択を変える | `Codex/README.md`「自走ルールと上限」。2026-09-15 の自走テストで Codex が指摘 |
| B15 | **受信箱の真正性（許可した Codex の会話だけ受け付けるか）** | 受信箱は Codex のサンドボックスから書けるので、別の Codex の会話からも Claude へ依頼を差し込める。Codex の案: 送信プロセスの PID を記録し、所有者・共有サーバー配下か・実行中のコマンドから thread UUID を特定して、待受時に指定した UUID と一致したものだけ受け付ける（不一致は `rejected/` へ隔離）。代償は、待受のたびに UUID を指定する手間と、Codex の内部 API への依存 | ユーザーが導入するか決める。導入までは、ブリッジ経由の破壊的・外部送信の操作を Claude 側でユーザーに確認する運用で抑える（`CLAUDE.md`・`~/.claude/CLAUDE.md` に記載済み） | 同上 |
