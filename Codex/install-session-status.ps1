...
try {
    ...
    Invoke-StatusRpc 'config/batchWrite' @{ edits = @(@{ keyPath = 'hooks.state'; value = $hookState; mergeStrategy = 'upsert' }, @{ keyPath = 'tui.terminal_title'; value = @('app-name', 'session-id', 'model-with-reasoning'); mergeStrategy = 'replace' }, @{ keyPath = 'tui.status_line'; value = @(); mergeStrategy = 'replace' }); reloadUserConfig = $true } 3 | Out-Null
