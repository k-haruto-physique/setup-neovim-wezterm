...
- 内蔵は `status_line = ["model-with-reasoning", "context-remaining", "thread-name", "project-name", "git-branch"]`。セッション名はディレクトリの直前。5 項目で実測約 96 桁なので 94 桁ペインでは末尾の `git-branch` から切れるが、名前は中ほどなので残る。**`thread-name` は `/rename` するまで表示されない**（"omitted when unnamed"）。`git-branch` はスレッドの活動時に解決されるため、起動直後は出ない。`terminal_title` は監視の結合キー（`session-id`）なので変更しない。
